...
import io.matthewnelson.differ.core.internal.LINE_BREAK
import io.matthewnelson.differ.core.internal.writeNewLine
...
internal constructor(
    @JvmField public val schema: Differ.Schema,
    ...
    internal companion object {
        ...
        internal fun BufferedSource.readDiffFileHeader(): Header {
            val versionString = readUtf8Line()
                ...
                ?: throw IllegalStateException("Failed to read DiffFile schema version")

            val schema = Differ.Schema.from(versionString)

            val createdAtString = readUtf8Line()
                ?.substringAfter(PREFIX_CREATED_AT)
                ?: throw IllegalStateException("Failed to read DiffFile createdAt")

            val createdAt = try {
                createdAtString.toInstant()
            } catch (e: IllegalArgumentException) {
                throw IllegalStateException("Failed to read DiffFile createdAt", e)
            }

            val forFileName = readUtf8Line()
                ?.substringAfter(PREFIX_FOR_FILE_NAME)
                ?: throw IllegalStateException("Failed to read DiffFile forFileNamed")

            val forFileHash = readUtf8Line()
                ?.substringAfter(PREFIX_FOR_FILE_HASH)
                ?: throw IllegalStateException("Failed to read DiffFile fileHash")

            val postApplyHash = readUtf8Line()
                ?.substringAfter(PREFIX_POST_APPLY_HASH)
                ?: throw IllegalStateException("Failed to read DiffFile postApplyHash")
